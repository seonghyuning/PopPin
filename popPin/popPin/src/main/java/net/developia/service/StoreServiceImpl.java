...
public class StoreServiceImpl implements StoreService{
    ...
    // 관리자가 팝업스토어 신청 목록 조회
    @Override
    public List<PopUpStoreVO> getPendingStores(int page, int size) {
        int startRow = (page - 1) * size + 1;
        int endRow = page * size;
        return storeMapper.getPendingStores(startRow, endRow);
    }
    
 // 관리자 팝업스토어 신청 목록
	@Override
	public List<PopUpStoreVO> getAddStoreList() {
		return storeMapper.getAddStoreList();
	}

    // 관리자 팝업스토어 신청 목록 페이징처리
    @Override
    public int getPendingStoresCount() {
        return storeMapper.getPendingStoresCount();
    }
    // 관리자 팝업스토어 신청 목록 전체 페이지 수 
    @Override
    public int getTotalDataCount() {
    	return storeMapper.getTotalDataCount();
    }
	...
	public List<ImageVO> getAllImages() {
		return storeMapper.getAllImages();
	}
