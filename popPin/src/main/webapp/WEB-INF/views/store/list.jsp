...
$(document).ready(function () {
    ...
    $(".card").on("click", function () {
        const storeId = $(this).data("store-id"); // 카드의 data-store-id 속성 값 가져오기
        console.log(storeId);
        if (storeId) {
            // Store 상세 페이지로 이동 (하트 버튼은 영향을 미치지 않음)
            window.location.href = `/store/storeDetail?storeId=\${storeId}`;
        }
    });

    // 하트 버튼 클릭 시 좋아요 상태만 토글
    $(".likeButton").on("click", function (event) {
        event.stopPropagation();  // 클릭 이벤트가 카드로 전파되지 않도록 방지
        const storeId = $(this).data("store-id");  // 하트 버튼의 데이터에서 storeId 가져오기
        toggleLike(storeId, this);  // 하트 버튼의 토글 함수 호출
    });
});
...
<body>
	...
	<div class="cardContainer">
		<c:forEach items="${stores}" var="store">
			<div class="card" data-store-id="${store.storeId}" style="cursor: pointer;">
				...
				<div class="cardContent">
					...
					<button class="likeButton"
						...
						<c:if test="${!likedMap[store.storeId]}">
							<!-- 비어있는 하트 -->
							<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
								fill="gray" class="bi bi-suit-heart" viewBox="0 0 16 16">
												<path
