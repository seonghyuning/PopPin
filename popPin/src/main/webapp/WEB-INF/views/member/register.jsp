...
<body>
	...
	<form method="post" action="/member/register" class="loginForm">
		...
		<label for="userName">아이디*</label> <input type="text" name="username"
			id="userName" required> <label for="email">이메일*</label> <input
			type="email" name="email" id="email" required> <label
			for="nickName">닉네임</label> <input type="text" name="nickname"
			id="nickName"> <label for="password">비밀번호*</label> <input
		...
		<p>
			이미 가입하셨나요 ? <a href="/member/login"
				style="text-decoration: underline;"><strong>로그인하기</strong></a>
		</p>
		<br>
